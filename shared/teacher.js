...
const STORE = `at_tutor_teacher_${QUIZ.id}`;
let rows = [];
...
function installTheme(){
  const saved=localStorage.getItem("at_tutor_theme") || "dark";
  document.documentElement.dataset.theme=saved;
  const btn=document.getElementById("themeToggle");
  if(btn){btn.textContent=saved==="light"?"🌙":"☀️"; btn.onclick=()=>{const cur=document.documentElement.dataset.theme==="light"?"dark":"light";document.documentElement.dataset.theme=cur;localStorage.setItem("at_tutor_theme",cur);btn.textContent=cur==="light"?"🌙":"☀️";};}
}
function normalizeAttempt(att, fallbackName){
  ...
  return {
    ...
    officialScore: typeof att.officialScore === "number" ? att.officialScore : null,
    importedAt: Date.now(),
...
function addPayload(payload){
  ...
  } else if(payload && (payload.type === "at-tutor-student-export" || payload.type === "cap-review-student-export")){
...
function render(){
  ...
  app.innerHTML = `
    <div class="spread"><div class="brand"><div class="logo">AT</div><div><h1>教師版｜${esc(QUIZ.title)}</h1><div class="sub">AT_tutor・全班成績與錯題統計</div></div></div><a class="btn sm ghost" href="index.html">學生端</a></div>
    ${cloudControls}
...
window.exportCSV = function(){
  const header=["name","date","mode","scorePercent","officialScore","correct","total","wrongIds","unitWeakness","source"];
  const lines=[header.join(",")];
  rows.forEach(r=>{
    const unitWrong={}; (r.wrongIds||[]).forEach(no=>{const q=QMAP[no]; if(q) unitWrong[q.unit]=(unitWrong[q.unit]||0)+1;});
    const vals=[r.name, fmtDate(r.date), r.mode, r.score, r.officialScore ?? "", r.correct, r.total, (r.wrongIds||[]).join(" "), Object.entries(unitWrong).map(([u,n])=>`${u}:${n}`).join(";"), dataSource];
    lines.push(vals.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(","));
  });
  const blob=new Blob(["\ufeff"+lines.join("\n")],{type:"text/csv;charset=utf-8"});
  const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download=`AT_tutor-${QUIZ.id}-results.csv`; a.click(); URL.revokeObjectURL(a.href);
};
installTheme();
if(cloudOn()){
  cloudInit();
